...
function saveReasonForCancellation(selectedReason, orderID) {
    ...
    $.ajax({
        url: "/cosmetic/save-reason-for-cancellation",
        type: "POST",
...
var checkAllOrderCalled = false;
var checkProcessingOrderCalled = false;
var checkShippedOrderCalled = false;
var checkDeliveredOrderCalled = false;

document.addEventListener('DOMContentLoaded', function() {
    if (!checkAllOrderCalled) {
        var elements = document.querySelectorAll('[check-all-order]');
        for (let element of elements) {
            if (element.getAttribute('check-all-order') === 'true' && !checkAllOrderCalled) {
                document.querySelector(".my-order-all").style.display = "none"
                break;
            }
        }
    }
    if (!checkProcessingOrderCalled) {
        var elements = document.querySelectorAll('[check-processing-order]');
        for (let element of elements) {
            if (element.getAttribute('check-processing-order') === 'true' && !checkProcessingOrderCalled) {
                document.querySelector(".my-order-processing").style.display = "none"
                break;
            }
        }
    }
    if (!checkShippedOrderCalled) {
        var elements = document.querySelectorAll('[check-shipped-order]');
        for (let element of elements) {
            if (element.getAttribute('check-shipped-order') === 'true' && !checkShippedOrderCalled) {
                document.querySelector(".my-order-shipped").style.display = "none"
                break;
            }
        }
    }
    if (!checkDeliveredOrderCalled) {
        var elements = document.querySelectorAll('[check-delivered-order]');
        for (let element of elements) {
            if (element.getAttribute('check-delivered-order') === 'true' && !checkDeliveredOrderCalled) {
                document.querySelector(".my-order-delivered").style.display = "none"
                break;
            }
        }
    }
